...
// Pequeño log para saber si estamos entrando a este archivo
file_put_contents(__DIR__ . '/debug_login_with.log',
    "\n==== [LOGIN-WITH.PHP START] ====\n" .
...
if (isset($provider) && in_array($provider, $types)) {
    try {
        ...
        if ($provider == 'OkRu') {
            ...
            // --------------------------------------------------
            // CAMBIO IMPORTANTE: callback limpio, sin parámetros
            // --------------------------------------------------
            require_once('./assets/libraries/tiktok/src/Connector.php');

            // Ajusta aquí tu dominio EXACTO (HTTPS), sin "?link1=" ni nada extra
            $callback = 'https://zuitch.com/tiktok_callback';
            ...
            if (Connector::receivingResponse()) {
                try {
                    ...
                    // Aquí tu lógica para manejar el Access Token y el perfil del usuario
                    // Por ejemplo:
        ...
        // --------------------------------------------------
        // BLOQUE GENÉRICO PARA OTROS PROVEEDORES
        // --------------------------------------------------
        else {
        ...
        // --------------------------------------------------
        // SI HAY PERFIL DE USUARIO, SEGUIMOS PROCESANDO
        // --------------------------------------------------
        if ($user_profile && isset($user_profile->identifier)) {
            ...
            // Si el email ya existe en la DB, inicias sesión
            if (Wo_EmailExists($user_email) === true) {
                ...
                // Diferentes campos según el proveedor
                if ($provider == 'Google') {
